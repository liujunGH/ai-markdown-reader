# 更新日志

## 未发布

---

## v2.1.0

### 长文档性能
- Markdown 改为约 64KB 的安全分段解析，首段完成即可显示正文，其余块索引在后台增量补齐。
- 块模型只保留源码与布局元数据；可见区及预取窗口才由独立 Worker 生成 HTML，并通过 160 块有界 LRU 控制内存。
- 同一文档的多视图共享解析请求和块模型；完整索引结束后预热尾部 20 块，提升滚动到底与阅读进度恢复速度。
- 百万字符以上正文搜索迁移到独立 Worker，文档统计改为单次低分配扫描，减少主线程长任务。
- 增量索引期间保持尾部锚定；Mermaid 或异步内容改变高度后重新测量可见块，避免后续内容重叠。

### 阅读体验
- 修复首次引导步骤与关闭状态、内置示例会话恢复、源码视图首行裁切、全局搜索空工作区入口等问题。
- 搜索高亮不再改写 Mermaid、KaTeX 和代码结构，关闭搜索时会清除残留标记。
- 主题切换后重新渲染 Mermaid；专注模式隐藏非阅读外壳并保留明确退出入口。
- macOS 命令面板显示原生快捷键符号，并清理冲突快捷键提示。

### 软件更新与发行
- 新增统一更新状态服务，支持静默检查、手动检查、发行说明、确认下载、下载进度、失败重试以及重启安装。
- 更新状态可跨多窗口查询；开发模式不会连接发布源，后台无更新或检查失败不会打断阅读。
- macOS 新增 ZIP 与 `latest-mac.yml` 更新资产；GitHub Actions 恢复 Windows、macOS、Linux 三平台构建发布矩阵。
- 升级 Electron 43、better-sqlite3 13、electron-builder 26、Vite 8、Vitest 4 和相关内容解析依赖，生产与完整 `npm audit` 均收敛为 0 项已知漏洞。
- README、发布流程、E2E 指南和性能脚本与当前实现同步。

### 验证
- 259 个单元测试与 8 个 Electron E2E 全部通过。
- TypeScript、Electron 主进程编译、Vite 生产构建和 macOS x64 ZIP/更新清单实包验证通过。

---

## v2.0.1

### 修复
- 修复 YAML frontmatter 被 markdown-it 渲染为普通文本的问题
- 补充 markdown 内容全局样式（标题、列表、间距、代码块、表格等），修复 v2 移除旧 MarkdownRenderer 后样式丢失
- 修复文件侧边栏遮挡标签栏的问题（调整 z-index 层级）
- 工具栏按钮添加中文 tooltip 提示
- 优化目录 Outline 面板间距与宽度

---

## v2.0.0

### 架构重构
- 从单体 MarkdownRenderer 迁移到分层架构：`tokenizer`（markdown-it 块级解析）→ `blockModel`（块边界切分）→ `DocumentView`（TanStack Virtual 虚拟化渲染）。
- 主线程渲染不再一次性注入整篇 HTML，改为按可见区间逐块渲染，大文档内存占用不再随阅读深度单调增长。
- `tabStore` 只存标签元数据，正文内容下沉到 `DocumentCache` LRU，响应式状态树不再承载 MB 级正文。

### 渲染与性能
- 新增 `@tanstack/react-virtual` 虚拟列表，滚动时只渲染可见块，滚出视口的块自动卸载。
- Markdown 解析在 Web Worker 中完成，worker 崩溃时降级到主线程同步解析，保证可用性。
- 块级 DOMPurify 净化替代整篇净化，大幅降低首次渲染长任务。
- 新增 `scripts/render-check.mjs`，用 Playwright Electron 验证 15 项渲染正确性（heading id、代码高亮、KaTeX、Mermaid、WikiLink、搜索高亮等）。

### 全文索引
- 全文索引从 IndexedDB 迁移到主进程 SQLite FTS5，使用 trigram tokenizer 支持中文 ≥3 字符子串匹配。
- 文件删除时通过外键级联与触发器可靠清理 FTS 索引与桥接记录，根治旧 IndexedDB 的索引膨胀。
- 阅读数据（高亮、稍后读、会话、章节、快照、书签）统一按 `file_path` 关联，具备 `ON DELETE CASCADE`。

### 安全与 IPC
- 主进程 IPC handler 按领域拆分（file / dialog / window / storage / db），channel 名、超时、限流集中管理。
- 所有文件路径经过 `validateFilePath` 校验，解析符号链接并限制在 home/userData/temp/desktop/documents/downloads 等安全根目录。
- `db:query` 只允许 SELECT 语句；`db:exec` 收紧为仅允许 INSERT/REPLACE/UPDATE/DELETE，拒绝 DDL 与连接配置变更。
- `watch-file` 支持多窗口同时监听同一文件，窗口关闭时自动清理对应 watcher。

### 修复
- 修复 Prism 语言并行加载顺序问题（tsx 依赖 jsx），解决 `Cannot convert undefined or null to object`。
- 修复 Minimap 参与 flex 布局导致文档可视高度为 0、虚拟列表无法渲染的问题。
- 修复首次使用引导未检查 localStorage、始终遮挡界面且无法交互的问题。
- 修复无 `filePath` 标签（欢迎页）在 `useDocument` 中抛错的问题。

### 工程化
- 单元测试 223 个、e2e 6 个全绿；新增 `render-check.mjs` 渲染验证。
- 移除 FileInfo / FilePreview / Timeline / IndexDiag / Workspace / recent / quickSwitcher 等旧面板与旧 worker 代码。

---

## v1.5.7

### 阅读优先收敛
- 移除偏知识库维护、发布运营和批量修复的重型面板，工具菜单回到阅读、查找、导航、索引诊断、文件信息和导出。
- 删除无入口旧组件、旧工具模块和对应测试，降低启动路径复杂度与维护成本。
- 欢迎页、README 和 docs 首页重新聚焦本地 Markdown 阅读。

### 启动与性能
- 会话恢复改为懒加载：启动时只恢复标签元数据，当前标签内容优先加载，其他文档切换时再读取。
- Markdown 渲染优先走 Web Worker，worker 失败时再回退到主线程解析。
- 启动阶段延后自动更新检查，避免自动恢复上次文件夹触发索引抢占首屏。
- 新增 `npm run perf:startup`，默认用 12 个 5MB 文档模拟大会话启动压测。

### 工程化
- 新增 `DocumentLoadState` 展示待加载、加载中和加载失败状态。
- `electron:dev` 改为等待 Vite 可用后启动 Electron，新增 `electron:preview`。
- 更新版本号与发布说明到 v1.5.7。
- 提供 macOS arm64 与 x64 DMG，自用版为 ad-hoc 签名。

---

## v1.5.5

### 工作区运营
- 新增“运营仪表盘”，集中展示工作区质量、可执行修复、链接重命名安全、图片资产、阅读助手和发布辅助
- 新增“待处理队列”，把缺失链接、图片问题、索引跳过和文档健康问题整理成可逐项处理的维护清单
- 新增“发布前检查”，汇总知识库健康分、待处理任务和索引覆盖率，支持复制 Markdown 检查报告
- 新增知识库健康报告、反向链接、文档图谱、缺失链接建议和阅读时间线等工作区维护工具

### 索引与链接
- 文件夹索引支持持久化诊断，记录被跳过的目录、大文件和读取失败项
- 新增可配置索引设置，可调整最大文件大小和跳过目录，并支持保存后立即重建索引
- WikiLink 和 Markdown `.md` 链接跳转更稳定，缺失链接支持候选匹配和创建目标文档
- 打开文件夹会记忆最近工作区，并降低长时间扫描带来的阻塞风险

### 图片与文档可靠性
- 新增文档健康检查面板，可检查危险链接、空链接、可疑图片、重复标题、Mermaid 空块和 KaTeX 定界符问题
- 新增图片检查面板，可汇总当前文档图片来源、分类、解析后的本地路径和常见警告
- 网络图片、本地图片、未解析图片会进入图片资产计划，方便后续本地化和路径修复

### 发布
- 更新项目文档，补齐 v1.5.5 新功能说明和本地 macOS 自用包说明

---

## v1.5.4

### 阅读体验
- 允许 Markdown 网络图片通过 CSP 加载，同时保持远程脚本、样式和 fetch 禁用
- 图片加载失败时显示可操作占位，支持复制路径、打开网络链接或在 Finder 中显示本地路径
- 本地图片路径支持安全的 `../` 父级相对路径，仍通过 realpath 边界校验防止符号链接逃逸
- 图片预览支持复制图片地址和打开网络原图

### 稳定性
- 启动恢复时静默移除已失效文件，不再每次打开弹出无效文件提示
- 最近文件打开失败后自动移除，并在最近文件页新增“清理失效”入口
- 导出 HTML 时会把本地图片内联为 `data:` URL，提升单文件归档可用性
- 命令面板新增“打开示例文档”，状态栏显示当前版本号

---

## v1.5.3

### 图片修复
- 修复 Markdown 相对路径本地图片无法显示的问题
- 本地图片现在通过安全 IPC 读取为 `data:` URL，不再依赖 `file://` 加载
- `examples/smoke-test.md` 改为使用离线本地图片夹具
- E2E 发布验收用例新增图片真实加载检查，断言图片 `naturalWidth > 0`

---

## v1.5.2

### 发布打磨
- 新增 `examples/smoke-test.md`，用于发布前快速验收 Markdown、Mermaid、KaTeX、代码高亮、表格、任务列表、WikiLink、图片和导出流程
- 统一应用内名称为 `Markdown Reader`，清理窗口标题、菜单、托盘、欢迎页和引导文案中的旧名称
- 更新关于窗口版本信息
- 清理 v1.5.1 Release 中重复的 Windows 安装包资产，保留自动更新配置引用的文件

---

## v1.5.1

### 产品收敛
- 按纯 Markdown 阅读器路线收敛功能范围
- 移除未落地的 AI、语义搜索、知识图谱、自动标签等文档承诺
- 移除代码块执行、诊断面板、数据备份、自定义 CSS、阅读统计等偏高级/偏开发能力
- 移除 `@xenova/transformers` 依赖，降低安装体积与维护复杂度

### 🧪 测试覆盖
- React 组件测试从 27 个扩展至 **99 个**
- 新增 Outline、ResizableSidebar、useOutline、useSearch、tabStore 测试
- 新增 MarkdownRenderer 组件测试（10 个测试用例）
- E2E 测试扩展至 **7 个**，新增 reading-flow 场景（文件打开、搜索、标签管理、主题切换、大纲导航）

### 🎨 界面与交互
- **启动画面** - Electron 主进程添加 splash window，提升应用启动体验
- **虚拟滚动优化** - 大文件（>300KB 或 >5000 行）自动启用虚拟滚动，显著降低内存占用
- **i18n 多语言支持** - 集成 i18next + react-i18next，支持中文/英文界面切换，自动检测系统语言

### ⌨️ 交互增强
- 命令面板保留阅读器核心命令：打开文件/文件夹、搜索、主题、导出、分屏、快速跳转

### 🔒 安全加固
- 替换 `markdown-it-katex` 为 `markdown-it-texmath + katex`，移除旧插件携带的高危 XSS 审计项
- KaTeX 渲染保持 `trust: false`，公式错误降级显示而不是中断渲染
- IPC 路径校验改为 realpath 边界判断，拒绝通过符号链接跳出用户安全目录

### ⚡ 性能优化
- **内存清理** - 关闭标签页时自动 strip 内容后再存入 `closedTabs`，降低历史标签内存占用
- **Worker 清理** - `useMarkdownWorker` 添加取消和 pending 任务清理机制
- **打包压缩** - `electron-builder.yml` 全平台启用 `compression: maximum`，减小安装包体积
- **构建提示降噪** - Mermaid 高级图表按需加载 ELK 异步 chunk，Vite chunk warning 阈值调整为匹配该已知产物，减少非首屏大 chunk 噪音

---

## v1.5.0

### 🚀 重大升级
- **Electron 28 → 35** - 升级至 Electron 35.7.5（Chromium 134, Node 22），享受最新安全补丁和性能改进
- **electron-builder 24 → 25** - 构建工具升级，修复 Apple Silicon 打包问题

### ⚡ 性能优化
- **Web Worker Markdown 解析** - markdown-it + Prism + KaTeX 全部 offload 到 Web Worker，主线程零阻塞
- **虚拟滚动优化** - 长文档渲染性能显著提升

### 🎨 界面与交互
- **CSS Design Tokens** - 统一 light/dark/sepia 三主题的设计令牌系统（primitive + semantic + spacing/font/radius/shadow/z-index）
- **布局重构** - TabBar + 工具栏移入 flex column，仅主内容区滚动，header/sidebar 移除 sticky
- **Sidebar 拖拽与折叠** - 左右侧边栏支持鼠标拖拽调整宽度，一键折叠/展开，宽度持久化到 localStorage
- **拖拽打开文件重写** - window 级别事件监听，支持整个窗口拖拽，计数器防止闪烁

### 🛠️ 新功能
- **代码块执行** - bash/sh/shell 代码块可直接运行（需先在设置中开启），内置命令白名单安全机制
- **诊断面板** - 应用信息、内存占用、最近日志一键查看（工具栏 🛠️ 按钮）
- **自动更新静默处理** - release 未发布时的 404 错误仅记录日志，不弹窗打扰用户

### 🧪 工程化
- **TypeScript Strict 模式** - 全部 3 个 tsconfig 启用 strict，零类型错误
- **Playwright E2E 测试** - 应用启动与基础交互的端到端测试
- **打包修复** - `electron-builder.yml` 正确包含 `lib/` 目录，修复 ad-hoc 签名兼容性

### 🐛 Bug 修复
- 修复右侧 sidebar 折叠后无展开按钮的问题
- 修复 Minimap 被拉伸导致的空白区域问题
- 修复空文档时 Outline 仍占据空间的空白问题
- 修复已删除文件缓存导致每次启动重复提示的问题（首次提示后自动清理）

---

## v1.4.0

### 🔒 安全加固
- 新增路径遍历防护：`isPathSafe()` 验证所有文件操作 IPC 路径
- 移除 `app.disableHardwareAcceleration()`，恢复 GPU 硬件加速
- 添加严格 Content-Security-Policy，禁止外部资源加载
- KaTeX CSS 改为本地打包，移除 CDN 依赖
- 禁用 Markdown raw HTML 渲染（`html: false`），收紧 DOMPurify 白名单
- Mermaid securityLevel 升级为 `strict`

### 🚀 性能优化
- Mermaid 按需加载，从 `manualChunks` 中移除预打包
- `parseMarkdown` 使用 `useMemo` 缓存，避免重复渲染
- Prism 语法高亮从 25+ 静态导入精简为 11 个常用语言，支持动态加载
- localStorage 瘦身：文件类标签不再重复存储完整 Markdown 内容

### 🏗️ 架构重构
- 提取 `useKeyboardShortcuts` Hook，`App.tsx` 瘦身 140+ 行
- 新增输入框快捷键拦截：聚焦在 `INPUT`/`TEXTAREA` 内时全局快捷键自动忽略
- 修复 `onOpenFolder` IPC listener 选中子目录的 bug

### 📦 打包与分发
- 支持 Apple Silicon (arm64) 原生构建
- 集成 `electron-updater` 自动更新
- 启用 ASAR 打包，保护源码
- CI 改为多平台矩阵构建（macOS / Ubuntu / Windows）
- 添加 `hardenedRuntime` 和 entitlements 配置框架

---

## v1.3.2

**修复:**
- 修复 macOS 打包后 `preload.js` 中 `require("path")` 在 sandbox 模式下报错，导致 `window.electronAPI` 不可用
- 修复 `open-file-dialog` / `read-file` / `handleFileOpen` 在路径为目录时抛出 `EISDIR` 错误
- 修复打开文件夹时若第一个条目是子目录导致失败的问题
- 修复 `pathBasename` 对以 `/` 结尾路径的处理
- 修复右侧 Minimap 宽度（48px → 32px）
- 包含 v1.3.1 的所有修复

---

## v1.3.1

**修复:**
- 修复 macOS 上无法通过菜单打开文件/文件夹的问题
- 修复主内容区域右侧留白、未占满窗口宽度的问题

---

## v1.3.0

### 桌面应用核心能力
- 新增：多窗口支持（`Ctrl+Shift+N` 新建窗口，窗口间拖拽标签）
- 新增：窗口状态持久化（位置、大小、最大化/全屏状态自动保存恢复）
- 新增：系统托盘（关闭最小化到托盘，右键菜单，点击切换显示/隐藏）
- 新增：Dock 图标与右键菜单（macOS）
- 新增：任务栏进度条（Windows）

### 阅读体验
- 新增：阅读进度记忆（每个文件独立保存/恢复滚动位置）
- 新增：独立文件阅读设置持久化（字体大小、源码视图、大纲展开按文件保存）
- 新增：阅读统计与热力图（追踪阅读时长，52周 GitHub 风格热力图）
- 新增：浮动目录（长文档滚动时左上角显示当前章节）
- 新增：文档内滚动历史（`Alt+←/→` 前进后退）
- 新增：源码模式行号显示与当前行高亮
- 新增：快速跳转（`Ctrl+G` 跳转到行号/标题/图片）

### Markdown 渲染增强
- 新增：代码块折叠（超过15行默认折叠，记忆状态）
- 新增：Diff 高亮（`diff` 代码块 `+` 绿色 / `-` 红色）
- 新增：任务列表交互（checkbox 可点击勾选，状态持久化）
- 新增：WikiLink 支持（`[[文件名]]` 自动链接）
- 新增：文本朗读 TTS（选中文字后 🔊 按钮朗读）
- 新增：数学公式源码提示（悬停显示 LaTeX 源码）
- 新增：图片懒加载
- 新增：Mermaid 主题联动（切换应用主题时图表自动重渲染）
- 新增：Mermaid 缓存（避免重复动态导入）

### 搜索与导航
- 新增：全局跨标签搜索（在所有打开标签中搜索）
- 新增：文件夹级全文搜索（`Ctrl+Shift+F` IndexedDB 全文索引）
- 新增：文件树搜索过滤（实时过滤，匹配高亮）
- 新增：搜索历史键盘导航（↑/↓ 选择历史记录）
- 新增：命令面板（`Ctrl+Shift+P` 16+ 条命令，模糊搜索）

### 标签页体验
- 新增：标签页固定 Pin（固定后不会被溢出淘汰）
- 新增：标签页颜色标记（6 种颜色，左侧竖线标识）
- 新增：标签页悬停预览（路径、大小、修改时间、内容预览）
- 新增：撤销关闭标签（`Ctrl+Shift+T`）
- 优化：标签页关闭 Toast 提示（显示恢复快捷键）

### 视觉与交互
- 新增：过渡动画系统（主题切换、标签切换、弹窗滑入、Toast 动画）
- 新增：自定义滚动条（Light/Dark/Sepia 三套主题适配）
- 新增：全局字体栈（PingFang SC、Microsoft YaHei 等中文字体）
- 新增：自定义 CSS 注入（实时编辑器，预设示例）
- 新增：拖拽视觉反馈（全屏遮罩 + 提示文字）
- 新增：骨架屏（会话恢复期间 shimmer 动画）
- 新增：迷你地图（右侧缩略导航条）
- 新增：空状态优化（插图 + 引导文案）
- 新增：打印样式（强制浅色、隐藏 UI）
- 新增：图片预览增强（滚轮缩放 0.25x~5x、拖动平移）
- 新增：双栏对比阅读（`Ctrl+\` 分屏显示两个文件）

### 主题与自动切换
- 新增：跟随系统自动切换主题（监听系统主题变化）
- 新增：按时间段自动深色（22:00-07:00）
- 新增：自定义 CSS 面板

### 导出
- 新增：导出为 HTML（自包含文件）
- 新增：导出为 PDF（调用打印对话框）
- 新增：复制为纯文本 / 富文本

### 无障碍与健壮性
- 新增：React 错误边界（组件崩溃时显示友好错误页）
- 新增：全面 ARIA 无障碍属性（11 个组件添加语义信息）
- 新增：DOMPurify XSS 防护（Markdown HTML 输出净化）
- 新增：localStorage 版本管理（数据迁移，防止升级崩溃）
- 新增：所有 storage 操作集中化（`storage.ts` 统一管理）
- 修复：ProgressBar 监听正确的滚动容器

### 工程化
- 重构：App.tsx 拆分（提取 useTabs / useRecentFiles / useFileWatcher / useDragAndDrop hooks）
- 重构：内联样式提取为 CSS 模块
- 新增：类型系统完善（`electron.d.ts` 统一声明 25+ IPC 方法）
- 新增：Vite Chunk 拆分（vendor-react / vendor-markdown / mermaid / katex / prism 独立打包）
- 新增：.gitignore 加强

---

## v1.2.2
- 修复：左边文件夹导航和右边目录导航固定可见，不随正文滚动消失

---

## v1.2.1
- 新增：Electron 主进程文件操作，支持真实文件路径
- 新增：原生文件拖拽支持
- 新增：外部文件变更检测
- 新增：标签页拖拽重排序
- 新增：专注模式隐藏操作栏
- 新增：文件预览面板
- 新增：最近文件全屏页面（保存100条历史）
- 新增：标签页数量限制（默认10个）
- 新增：标签栏和工具栏固定在顶部，不随滚动消失
- 新增：md 链接在新标签页打开，外部链接在系统浏览器打开
- 新增：Mermaid 放大预览，支持缩放（25%~500%）和拖动移动
- 新增：Mermaid 导出文件根据文档位置自动命名
- 新增：空文件夹引导页
- 新增：文件夹恢复记忆
- 优化：文件夹浏览器支持子目录结构展示
- 优化：文件变更标记和刷新功能
- 优化：StatusBar 固定显示

---

## v1.2.0
- 新增：Electron 主进程文件操作，支持真实文件路径
- 新增：原生文件拖拽支持
- 新增：外部文件变更检测
- 新增：标签页拖拽重排序
- 新增：专注模式隐藏操作栏
- 新增：文件预览面板
- 新增：最近文件全屏页面（保存100条历史）
- 新增：标签页数量限制（默认10个）
- 优化：文件夹浏览器支持子目录结构展示
- 优化：文件变更标记和刷新功能
- 优化：StatusBar 固定显示

---

## v1.1.6
- 修复：标签页右键菜单被遮挡问题
- 新增：复制文件路径功能
- 新增：在 Finder 中显示功能
- 新增：快速切换器路径搜索
