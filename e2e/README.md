...
3. **前端生产构建产物已生成**
   ...
   > 如果 `dist-electron/electron/main.js` 或 `dist/index.html` 不存在，测试会自动启动失败，提示需要先执行编译和构建。
...
- **测试目录**: `e2e/`
- **测试入口**: `dist-electron/electron/main.js`（编译后的 Electron 主进程）
...
2. 随后通过 `_electron.launch()` 启动 Electron 应用，主进程入口指向 `dist-electron/electron/main.js`。
3. Electron 窗口加载 `http://localhost:5173`（即生产构建产物）。
4. 测试执行断言（文件阅读、模式切换、软件更新 IPC 与界面状态等）。
